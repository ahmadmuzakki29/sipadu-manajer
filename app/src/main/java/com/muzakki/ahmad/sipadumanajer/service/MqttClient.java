...
import com.muzakki.ahmad.sipadumanajer.main.Aduan;
 ...
 * android com.muzakki.ahmad.sipadumanajer.service to actually interface with MQTT server. It provides android applications a simple programming interface to all features of the MQTT version 3.1
...
public class MqttClient extends BroadcastReceiver {
    ...
    /**
     * ServiceConnection to process when we bind to our com.muzakki.ahmad.sipadumanajer.service
     */
    ...
    public IMqttToken connect(MqttConnectOptions options, Object userContext,
		...
		/*
		 * The actual connection depends on the com.muzakki.ahmad.sipadumanajer.service, which we start and bind
		 ...
		 */
        if (mqttService == null) { // First time - must bind to the com.muzakki.ahmad.sipadumanajer.service
            ...
            if (service == null) {
                ...
                if (listener != null) {
                    listener.onFailure(token, new RuntimeException(
                            "cannot start com.muzakki.ahmad.sipadumanajer.service " + SERVICE_NAME));
                }
            }
     ...
     * @param qos
     *            the maximum quality of com.muzakki.ahmad.sipadumanajer.service at which to subscribe. Messages
     *            published at a lower quality of com.muzakki.ahmad.sipadumanajer.service will be received at
     *            the published QoS. Messages published at a higher quality of
     *            com.muzakki.ahmad.sipadumanajer.service will be received using the QoS specified on the
     *            subscription.
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (MqttServiceConstants.CONNECT_ACTION.equals(action)) {
            ...
            String latest = data.getString("latest");

            if(type.equals("inbox")){
                ((Aduan) myContext).updateInbox(latest);
            }else{
                ((Aduan) myContext).updateOutbox(latest);
            }
        }
    ...
    private void connectAction(Bundle data) {
        ...
        if(!isConnected()){
            Log.i("jeki","not connected cannot subscribe");
            return;
        }
        ...
        try {
            ...
            subscribe(Constants.TOPIC + username, 0);
